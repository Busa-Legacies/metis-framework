...
GOALS = [
    {"id": "G1", "system": 1, "title": "Ecosystem", "weight": 5, "areas": ["OpenClaw Infrastructure", "Automation"]},
    {"id": "G2", "system": 1, "title": "Dashboard", "weight": 4, "areas": ["Dashboard"]},
    {"id": "G3", "system": 2, "title": "Navore", "weight": 4, "areas": ["Navore Market"]},
]


def task(tid, area=None, pri="P2", state="queued", blocker="none", goalIds=None):
...
tasks = [
    ...
    task("#nav", "Navore Market", "P1"),                   # 12, system 2
...
check("system 2 holds navore", [s["taskId"] for s in r["by_system"][2]] == ["#nav"])
...
check("G3 active from navore", cov["G3"]["active"] == 1)

print("== real data sanity ==")
try:
    g, t = p.load_goals(), p.load_tasks()
    rr = p.rank(t, g)
    check("loads real goals.json + tasks.json", len(g) == 5 and len(t) > 0)
