...
def render_projects(projects_doc) -> str:
    ...
    # Goal roll-up (priority-weighted) — #221.
    goals = sorted({p.get("goal") for p in projects if p.get("goal")})
    if _progress is not None and goals:
        out.append("## Goal roll-up (priority-weighted; excludes evergreen/paused)\n\n")
        for g in goals:
            gp = _progress.goal_rollup(projects, g)
            if gp is None:
                out.append(f"- **{g}** — (no seeded active projects)\n")
            else:
                out.append(f"- **{g}** — {_progress.bar(gp)} {gp * 100:.0f}%\n")
        out.append("\n")
