...
if [ "$r7" -ge 95 ] && [ "$_bypassed" -eq 0 ] && ! _resets_soon "${RATE_7D_RESETS:-}"; then
  ...
  rate_msg="WEEKLY RATE LIMIT: 7-day window at ${r7}% — the scarce cap. Be maximally economical with Claude-API calls: route ALL generation, research, review, and drafting to <<MACHINE_1_ID>>'s Ollama lanes (smith/scout/warden/echo) and apply output inline. Reserve Claude for orchestration, git, and applying lane output."
...
if [ "$r5" -ge 85 ]; then
  rate_msg="RATE LIMIT FALLBACK: 5-hour window at ${r5}% (it recovers continuously). Switch to local-model fallback NOW: route code generation, research, review, and drafting to <<MACHINE_1_ID>>'s Ollama lanes (smith/scout/warden/echo) and apply output inline. Reserve Claude-API for orchestration, git, and applying lane output. Be concise; avoid unnecessary tool calls."
  additional_ctx="${additional_ctx:+${additional_ctx} }${rate_msg}"
  printf 'HIGH\n' > "$_rl_state_file"
elif [ "$r5" -ge 70 ]; then
  rate_msg="RATE LIMIT NOTICE: 5-hour window at ${r5}%. Be concise and efficient; avoid unnecessary tool calls. Prefer routing generation to <<MACHINE_1_ID>>'s Ollama lanes (smith/scout/warden)."
  additional_ctx="${additional_ctx:+${additional_ctx} }${rate_msg}"
  printf 'HIGH\n' > "$_rl_state_file"
else
