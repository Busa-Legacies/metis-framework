/**
 * Per-account Claude credentials for spawned `claude` CLIs (Navore professional
 * workspace vs Ant's default). Two Claude accounts on one machine:
 *
 *   - `default`  — the machine's own `~/.claude` login (no overrides).
 *   - `navore`   — a separate Claude subscription, run via env overrides so it
 ...
 * spawn (plus `CLAUDE_CONFIG_DIR` to isolate settings/history). So the Navore
...
export type ClaudeAccount = 'default' | 'navore'

export interface ClaudeAccountSource {
  /** Operator home dir, for the default Navore config-dir location. */
  home: string
  /** AW_CLAUDE_NAVORE_CONFIG_DIR — overrides the default `~/.claude-navore`. */
...
function navoreDir(src: ClaudeAccountSource): string {
  ...
  return `${src.home.replace(/\/$/, '')}/.claude-navore`
}

/**
 * Env overrides to run a spawned agent under a given account. Only `claude`
 * agents under the `navore` account get overrides — everything else (other
...
export function claudeAccountEnv(
  ...
  if (kind !== 'claude' || account !== 'navore') return {}
...
/**
 * Is the Navore account usable? On macOS that means a long-lived token is
...
export function accountForWorkspace(workspace: 'personal' | 'professional'): ClaudeAccount {
  return workspace === 'professional' ? 'navore' : 'default'
}
