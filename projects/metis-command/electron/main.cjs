...
const NEXT_PORT = Number(process.env.AW_NEXT_PORT || 3747)
const PTY_PORT = Number(process.env.AW_PTY_PORT || 3748)
...
const CHILD_LOG_DIR = path.join(os.homedir(), '.openclaw', 'metis-command', 'electron-logs')
...
app.on('ready', async () => {
  ...
  // Global hotkey for the floating Metis Brain HUD
...
ipcMain.handle('aw:capture-preview', async (_e, rect) => {
