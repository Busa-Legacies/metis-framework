 ...
 * Runs on AW_PTY_PORT (default 3748). Persists workspaces to ~/.openclaw/metis-command/state.json.
 */
...
const PORT = Number(process.env.AW_PTY_PORT ?? 3748)
    ...
    case 'codex': {
      ...
      if (ctx.initialPrompt && ctx.initialPrompt.trim()) {
        return {
          ...
          args: ['exec', '--sandbox', 'danger-full-access', '--ask-for-approval', 'never', ctx.initialPrompt],
        }
      }
      if (ctx.resume) return { cmd: process.env.AW_CODEX_CMD ?? 'codex', args: ['--sandbox', 'danger-full-access', '--ask-for-approval', 'never', 'resume', '--last'] }
      return { cmd: process.env.AW_CODEX_CMD ?? 'codex', args: ['--sandbox', 'danger-full-access', '--ask-for-approval', 'never'] }
    }
    case 'gemini':
...
function spawnAgent(input: {
  ...
  const navTok = navoreOAuthToken()
  Object.assign(env, claudeAccountEnv(input.account, input.kind, {
    home: os.homedir(),
