  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end" onMouseDown={onClose}>
      ...
      <aside
        ...
        className="relative z-50 flex h-full w-full max-w-md flex-col border-l border-slate-400/20 bg-[#0b0e15] shadow-2xl"
      >
        <header className="flex items-center justify-between border-b border-slate-400/10 px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <div className="text-sm font-bold uppercase tracking-[0.22em] text-cyan-100">Settings</div>
          </div>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-5 py-5">
                  ...
                  <div key={row.kind} className="rounded-lg border border-slate-400/15 bg-black/30 p-3">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 rounded-md border border-cyan-300/30 bg-cyan-300/10 px-2.5 py-1 text-xs text-cyan-100 hover:bg-cyan-300/20 disabled:opacity-40"
                      >
          ...
          <section>
            ...
            <p className="mb-3 text-xs leading-5 text-slate-500">Which provider runs the in-app chat panel that spawns agents on your behalf.</p>
            <div className="grid grid-cols-2 gap-2">
              {([
                ...
                <button
                  ...
                  className={`rounded-lg border px-3 py-2 text-left text-xs transition ${provider === opt.id ? 'border-cyan-300/40 bg-cyan-300/10 text-white' : 'border-slate-400/15 bg-black/30 text-slate-300 hover:text-white'}`}
                >
          ...
          <section className={provider === 'openai' ? '' : 'opacity-60'}>
            ...
            <p className="mb-3 text-xs leading-5 text-slate-500">Only needed if you set Assistant Brain to "OpenAI Key". Stored at <code className="text-slate-300">~/.openclaw/metis-command/settings.json</code>.</p>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-md border border-slate-400/15 bg-black/40 px-2 py-1.5 text-sm text-white placeholder:text-slate-500 focus:border-cyan-300/40 focus:outline-none"
              />
              <button onClick={() => save({ openaiApiKey: keyInput })} disabled={!keyInput || saving} className="rounded-md border border-cyan-300/30 bg-cyan-300/10 px-3 py-1.5 text-xs text-cyan-100 hover:bg-cyan-300/20 disabled:opacity-50">
                {saving ? <Loader2 size={12} className="animate-spin" /> : 'save'}
              </button>
              {s.hasOpenAIKey && (
                <button onClick={() => save({ clearOpenAI: true })} className="rounded-md border border-rose-300/30 bg-rose-300/10 px-3 py-1.5 text-xs text-rose-200 hover:bg-rose-300/20">
                  clear
                </button>
              )}
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2">
              <div>
                ...
                <input value={model} onChange={(e) => setModel(e.target.value)} onBlur={() => model && save({ assistantModel: model })} className="w-full rounded-md border border-slate-400/15 bg-black/40 px-2 py-1.5 text-xs text-white focus:border-cyan-300/40 focus:outline-none" />
              </div>
              <div>
                <label className="mb-1 block text-[10px] uppercase tracking-[0.18em] text-slate-500">Fallback</label>
                <input value={fallback} onChange={(e) => setFallback(e.target.value)} onBlur={() => fallback && save({ fallbackModel: fallback })} className="w-full rounded-md border border-slate-400/15 bg-black/40 px-2 py-1.5 text-xs text-white focus:border-cyan-300/40 focus:outline-none" />
              </div>
            </div>
  ...
  return (
    <div>
      ...
      <div className="flex gap-2">
        ...
        />
        <button onClick={async () => { if (val.length >= 12) { await onSave({ bridgeApiKey: val }); setVal('') } }} disabled={val.length < 12} className="rounded-md border border-cyan-300/30 bg-cyan-300/10 px-3 py-1.5 text-xs text-cyan-100 hover:bg-cyan-300/20 disabled:opacity-50">save</button>
        {s.hasBridgeKey && (
          <button onClick={() => onSave({ clearBridge: true })} className="rounded-md border border-rose-300/30 bg-rose-300/10 px-3 py-1.5 text-xs text-rose-200 hover:bg-rose-300/20">clear</button>
        )}
      </div>
    </div>
  )
